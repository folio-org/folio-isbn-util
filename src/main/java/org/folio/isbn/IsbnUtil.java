...
public final class IsbnUtil {
  ...
  private static final Logger logger = LogManager.getLogger(IsbnUtil.class);
  ...
  private static final ISBNFormat ISBN_FORMATTER = new ISBNFormat();
  ...
  public static String insertHyphens(String isbn) {
    ...
    try {
      return ISBN_FORMATTER.format(isbn, HYPHEN_GROUP_SEPARATOR);
    } catch (ISBNException e) {
      logger.warn("Error during call isbnFormatter.format with parameters input '{}', groupSeparator '{}' - {}", isbn,
        HYPHEN_GROUP_SEPARATOR, e.getMessage());
